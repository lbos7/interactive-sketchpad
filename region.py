...
class Region:
    ...
    def __init__(self, pos, size, color, text="", transparency=1.0):
        self.pos = pos
        self.size = size
        self.color = color
        self.text = text
    ...
    def draw(self, img, text_color=(255, 255, 255), font=cv2.FONT_HERSHEY_TRIPLEX, font_scale=1.0, thickness=2):
        bg_rect = img[self.pos[0]:self.pos[0] + self.size[0],
                      self.pos[1]:self.pos[1] + self.size[1]].astype(np.float32)
        reg_rect = np.ones(self.size) * np.array(self.color, dtype=np.float32)
        ...
        text_pos = (int(self.pos[1] + self.size[1] - text_size[0][0] / 2),
                    int(self.pos[0] + self.size[0] + text_size[0][1] / 2))
